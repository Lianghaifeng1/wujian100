...
// A configurable boundary-biased distribution constraint for numeric variables.
//
// By default, this biases randomization towards the two ends of the range with a 2:1:2
// low-edge:middle:high-edge weight split.
// Callers must ensure MIN_ <= MAX_ and EDGE_SIZE_ >= 1.
`ifndef DV_BOUNDARY_DIST_CONSTRAINT
`define DV_BOUNDARY_DIST_CONSTRAINT(VAR_, MIN_, MAX_, EDGE_SIZE_ = 1, EDGE_W_ = 2, MID_W_ = 1) \
  if ((MIN_) == (MAX_)) { \
    VAR_ == (MIN_); \
  } else if ((2 * (EDGE_SIZE_)) < (((MAX_) - (MIN_)) + 1)) { \
    VAR_ dist { \
      [(MIN_):((MIN_) + (EDGE_SIZE_) - 1)]   :/ (EDGE_W_), \
      [((MIN_) + (EDGE_SIZE_)):((MAX_) - (EDGE_SIZE_))] :/ (MID_W_), \
      [((MAX_) - (EDGE_SIZE_) + 1):(MAX_)]   :/ (EDGE_W_) \
    }; \
  } else if ((2 * (EDGE_SIZE_)) == (((MAX_) - (MIN_)) + 1)) { \
    VAR_ dist { \
      [(MIN_):((MIN_) + (EDGE_SIZE_) - 1)] :/ (EDGE_W_), \
