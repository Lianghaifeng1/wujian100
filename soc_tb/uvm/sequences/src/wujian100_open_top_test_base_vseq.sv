...
task wujian100_open_top_test_base_vseq::pre_body();
  ...
  if (!uvm_config_db#(virtual clk_rst_if)::get(m_sequencer, "", "hclk_rst_vif", hclk_rst_vif)) begin
    `uvm_fatal(get_full_name(),"Getting hclk_rst_vif failed,please check it!")
  end
  hclk_rst_vif.apply_reset();
